...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let send_notification_on_plan_created_subscriber: Box<dyn DomainEventSubscriber> =
        SendNotificationOnPlanCreatedSubscriber::new(Arc::clone(&notification_service));
    let send_notification_on_plan_completed_subscriber: Box<dyn DomainEventSubscriber> =
        SendNotificationOnPlanCompletedSubscriber::new(Arc::clone(&notification_service));
    let send_notification_on_todo_added_subscriber: Box<dyn DomainEventSubscriber> =
    ...
    backoffice_context
        .plan_aggregate
        .create_plan(CreatePlanRequestObject {
    ...
    backoffice_context
        .plan_aggregate
        .find_plans(FindPlansRequestObject {
            limit: 10,
            offset: 0,
    ...
    let plan_id = backoffice_context
        .plan_aggregate
        .caught_plan_id
        .lock()
        .unwrap()
    ...
    backoffice_context
        .plan_aggregate
        .add_todo(AddTodoRequestObject {
            plan_id: plan_id.clone(),
            description: "My First Todo".to_string(),
        })
        .await;

    let todo_id = backoffice_context
        .plan_aggregate
        .caught_todo_id
        .lock()
        .unwrap()
    ...
    backoffice_context
        .plan_aggregate
        .update_todo(UpdateTodoRequestObject {
            plan_id: plan_id.clone(),
    ...
    backoffice_context
        .plan_aggregate
        .check_todo(CheckTodoRequestObject {
            plan_id: plan_id.clone(),
            todo_id: todo_id.clone(),
        })
        .await;
    ...
    backoffice_context
        .plan_aggregate
        .remove_todo(RemoveTodoRequestObject {
            plan_id: plan_id.clone(),
            todo_id: todo_id.clone(),
        })
        .await;
